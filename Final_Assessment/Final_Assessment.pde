...
color background, overLay, line, point, textColour;
...
String randomTitle() {
  ...
  String randTitle = firstWord[int(random(firstWord.length))] + " " + secondWord[int(random(secondWord.length))];
...
void randomise() {
  ...
  background = colour[int(random(colour.length))];
  overLay = colour[int(random(colour.length))];
